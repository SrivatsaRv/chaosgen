...
def demo_inventory_fetch():
    ...
    try:
        ...
        print(f"📊 Cluster: {topology.get('metadata', {}).get('cluster', {}).get('kubernetes_version', 'Unknown')}")
...
def demo_monitoring(run_id, experiment):
    ...
    try:
        ...
        for i in range(6):  # 6 updates over 60 seconds
            ...
            print(f"   📈 Update {i+1}/6: Error Rate: {error_rate:.3f}, Latency: {latency:.3f}s")
            ...
            if abort_threshold.get('metric') == 'error_rate' and error_rate > abort_threshold.get('value', 0.05):
                print(f"   🛑 Abort condition met: Error rate {error_rate:.3f} > {abort_threshold.get('value', 0.05)}")
                break
